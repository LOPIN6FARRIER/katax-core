...
export class FileSchema extends BaseSchema<File> {
  ...
  _parse(input: unknown, path: Issue["path"]): Issue[] | File {
  ...
  extensions(exts: string[], message?: string): this {
    ...
    this.rules.push({
      check: (file) => extensions.some(ext => 
